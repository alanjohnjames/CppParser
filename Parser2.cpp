// add a parser combinator to identify variable names

#include <iostream>
#include <string>
#include <optional>
#include <functional>
#include <cctype>

using namespace std;

// Define a parser combinator function type
using Parser = function<optional<string>(const string&)>;

// Parser combinator function to parse a single character
Parser charParser(char c) {
    return [c](const string& input) -> optional<string> {
        if (!input.empty() && input[0] == c) {
            return string(1, c);
        } else {
            return nullopt;
        }
    };
}

// Parser combinator to parse variable names
Parser variableNameParser() {
    return [](const string& input) -> optional<string> {
        if (input.empty() || !isalpha(input[0])) {
            return nullopt; // Variable name must start with a letter
        }

        size_t length = 1;
        while (length < input.size() && (isalnum(input[length]) || input[length] == '_')) {
            length++;
        }

        return input.substr(0, length);
    };
}

// Test the parser
int main() {
    string input = "abc_123 x y_2";

    // Parse variable names
    auto parser = variableNameParser();
    size_t startPos = 0;
    while (startPos < input.size()) {
        optional<string> result = parser(input.substr(startPos));
        if (result) {
            cout << "Parsed variable name: " << result.value() << endl;
            startPos += result.value().size();
        } else {
            // Skip invalid characters
            startPos++;
        }
    }

    return 0;
}


